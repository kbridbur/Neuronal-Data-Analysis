  ...
  for section = 1:numSections
    ...
    for index = 1:length(sectionList)
      if sectionList(index) > (average+stdev*PeakCutoff)
        Peaks = [Peaks [1; currentSection*lengthSections+index/InputHz; sectionList(index)]];
      end
      if sectionList(index) < (average+stdev*PeakCutoff)
        Peaks = [Peaks [0; currentSection*lengthSections+index/InputHz; NaN]];
      end
    end
